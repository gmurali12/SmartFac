 ...
 */
// @WebServlet("/getSmartInsertDetails")
public class SmartInsertServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		String chartData = getSmartInsertChartData(request, response);
		String tableData = getSmartInsertTableData(request, response);
	...
	private String getSmartInsertChartData(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {

		long startTime = System.currentTimeMillis();
		System.out
				.println("Inside getSmartInsertChartData - START " + startTime);
		JSONArray jsonArray = null;
		...
		try {
			...
			URL url = new URL("" + SmartConstant.AGGREGATE_TIMESERIES_URL
					+ assetId + SmartConstant.SMART_INSERT_ASPECT_NAME
					+ SmartConstant.SMART_INSERT_CHART + dateFilter);
			...
			if(myURLConnection.getResponseCode() == 400){
				...
				System.out.println("Inside getSmartInsertChartData - Time Taken: "
	...
	private String getSmartInsertTableData(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {

		long startTime = System.currentTimeMillis();
		System.out
				.println("Inside getSmartInsertTableData - START " + startTime);
		JSONArray jsonArray = null;
		...
		try {
			...
			if(myURLConnection.getResponseCode() == 400){
				PrintWriter out = response.getWriter();
				response.setContentType("application/json");
				out.print("BAD_REQUEST, interval not aligned");
				...
				System.out.println("Inside getSmartInsertTableData - Time Taken: "
						+ (System.currentTimeMillis() - startTime) / 1000
						+ " seconds");
			}
